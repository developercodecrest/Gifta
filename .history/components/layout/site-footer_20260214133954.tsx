      ...
      <div className="border-b border-border bg-background">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-5 sm:px-6 lg:px-8">
          <div>
            ...
            <h3 className="mt-1 text-xl font-bold">Need help choosing the perfect gift?</h3>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link href="/store" className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">
              Shop now
            </Link>
            <Link href="/store?category=Corporate" className="rounded-lg border border-border px-4 py-2 text-sm font-semibold">
              Corporate gifting
            </Link>
          </div>
        </div>
...
function TrustPill({
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-md border border-border bg-background px-2 py-1 text-xs text-muted">
      ...
      <ul className="mt-3 space-y-2.5 text-sm text-muted">
        {links.map((item) => (
          <li key={item.href + item.label}>
            <Link href={item.href} className="inline-flex rounded-sm transition hover:text-foreground">
              {item.label}
            </Link>
          </li>
        ))}

    ...
    <header className="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur">
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-2 rounded-full border border-border bg-white p-1 md:flex">
          {navItems.map((item) => (
            <Link
              ...
              href={item.href}
              className={cn(
                "rounded-full px-3 py-1.5 text-sm font-medium text-muted transition hover:text-foreground",
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/auth/sign-in"
            className="hidden rounded-md border border-border px-3 py-2 text-sm font-medium md:inline-flex"
          >
            Sign in
          </Link>
        </div>

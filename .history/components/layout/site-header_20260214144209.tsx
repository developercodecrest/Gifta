    ...
    <header className="sticky top-0 z-40 border-b border-[#d6d6d6] bg-[#f3f3f3]/95 backdrop-blur">
      <div className="mx-auto flex max-w-[1280px] items-center justify-between gap-3 px-3 py-3 sm:px-5 lg:px-8">
        <div className="flex min-w-0 items-center gap-3 lg:gap-4">
          ...
          <Link href="/" className="text-3xl font-black lowercase leading-none tracking-tight text-[#f1467d]">
            gifta
          </Link>
      ...
      {open && (
        <div className="border-t border-[#dadada] bg-white px-4 py-4 shadow-[0_8px_24px_rgba(0,0,0,0.08)]">
          <nav className="mx-auto grid max-w-[1280px] gap-2 sm:grid-cols-2 lg:grid-cols-4">
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  "inline-flex min-h-11 items-center rounded-md border border-[#ececec] px-3 text-sm font-medium text-[#2f2f2f] transition hover:bg-[#f6f6f6]",
                  isActiveLink(pathname, item.href) && "border-[#d8d8d8] bg-[#f1f1f1]",
                )}
              >
                {item.label}
              </Link>

...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-background/95 shadow-[0_1px_0_0_rgba(0,0,0,0.02)] backdrop-blur">
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-4 py-3 sm:gap-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          <Link href="/" className="text-2xl font-bold tracking-tight text-primary sm:text-[1.7rem]">
            Gifta
          </Link>
        ...
        <nav className="hidden items-center gap-1.5 rounded-full border border-border bg-white p-1 md:flex">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
      ...
      <div className="border-t border-border bg-white/80">
        <form action="/store" method="get" className="mx-auto flex max-w-7xl items-center gap-2 px-4 py-3 sm:px-6 lg:px-8">
          <div className="relative w-full">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#2f3a5e]/60" />
            <input
              name="q"
              placeholder="Search gifts, flowers, hampers, tags..."
              className="min-h-11 w-full rounded-lg border border-[#edd2d9] bg-white pl-10 pr-3 text-sm outline-none focus:border-[#24438f]"
            />
          </div>
          <button
            type="submit"
            className="min-h-11 shrink-0 rounded-lg bg-[#24438f] px-4 text-sm font-semibold text-white"
          >
            Search
          </button>
        </form>
      ...
      {open && (
        <div className="space-y-3 border-t border-border px-4 py-4 md:hidden">
          <form action="/store" method="get" className="flex items-center gap-2">
            <input
              name="q"
              placeholder="Search items"
              className="min-h-11 w-full rounded-lg border border-[#edd2d9] bg-white px-3 text-sm"
            />
            <button type="submit" className="min-h-11 rounded-lg bg-[#24438f] px-4 text-sm font-semibold text-white">
              Go
            </button>
          </form>

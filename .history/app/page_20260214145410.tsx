...
export default function Home() {
  ...
  return (
    <div className="space-y-12 sm:space-y-14">
      <section className="overflow-x-auto rounded-xl border border-[#ef3f7a] bg-[#f5f5f5]">
          ...
          {quickFilters.map((item, index) => {
            ...
            return (
              <Link
                key={item.label}
                href={item.href}
                className="group relative flex min-h-28 flex-col items-center justify-center gap-3 px-4 text-center"
              >
                {index !== quickFilters.length - 1 && (
                  <span className="pointer-events-none absolute right-0 top-4 h-[calc(100%-2rem)] w-px bg-[#ef3f7a]" />
                )}
                <span className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-[#f6dbe4] text-[#5b5b5b]">
                  <Icon className="h-6 w-6" />
                </span>
                <span className="text-[1.75rem] text-xl font-medium text-[#24304e] transition group-hover:text-[#ef3f7a]">
                  {item.label}
                </span>
              </Link>
            );

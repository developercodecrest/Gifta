    ...
    <div className="grid gap-6 sm:gap-8 lg:grid-cols-[2fr_1fr]">
        ...
        {mapped.length === 0 ? (
          ...
          <div className="space-y-4">
            {mapped.map(({ product, quantity, subtotal: lineTotal }) => (
              <article key={product.id} className="grid grid-cols-[96px_1fr] gap-3 rounded-2xl border border-border bg-white p-3 sm:grid-cols-[120px_1fr_auto] sm:gap-4 sm:p-4 sm:items-center">
                ...
                <div className="col-span-2 space-y-3 sm:col-span-1">
                  <div className="inline-flex min-h-10 items-center rounded-md border border-border">
                  ...
                  <p className="text-sm font-semibold sm:text-right">{formatCurrency(lineTotal)}</p>
                  <button onClick={() => removeItem(product.id)} className="flex items-center gap-1 text-xs text-muted sm:ml-auto" type="button">
      ...
      <aside className="h-fit rounded-2xl border border-border bg-surface p-4 sm:p-5 lg:sticky lg:top-24">
        ...
        <Link
          ...
          className="mt-5 inline-flex min-h-11 w-full justify-center rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground"
        >

...
export function UsersClient({ users }: { users: AdminUser[] }) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-2.5 rounded-[1.25rem] border border-border/70 bg-card/85 p-3.5 lg:grid-cols-[minmax(0,1.4fr)_auto_auto]">
      ...
      {!sorted.length ? (
              ...
              <tbody>
                {sorted.map((user) => (
                  <tr key={user.userId} className="border-t border-border">
                    <td className="px-3 py-2.5 font-medium">{user.fullName}</td>
                    <td className="px-3 py-2.5 text-[#5f5047]">{user.email}</td>
                    <td className="px-3 py-2.5 text-[#5f5047]">{user.phone ?? "-"}</td>
                    <td className="px-3 py-2.5 text-[#5f5047]">{new Date(user.updatedAt).toLocaleString()}</td>
                    <td className="px-3 py-2.5"><div className="flex justify-end"><UserRowActions user={user} /></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="space-y-3">
          {sorted.map((user) => (
            <Card key={user.userId} className="border-border/70 bg-background/80"><CardContent className="flex min-h-28 items-start justify-between gap-3 p-3.5">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {sorted.map((user) => (
            <Card key={user.userId} className="border-border/70 bg-background/80"><CardContent className="flex min-h-36 flex-col space-y-2 p-4">
              <p className="font-semibold">{user.fullName}</p>
              <p className="text-sm text-[#5f5047]">{user.email}</p>

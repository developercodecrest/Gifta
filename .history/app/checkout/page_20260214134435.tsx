  ...
  if (!mapped.length) {
    return (
      <div className="rounded-2xl border border-dashed border-border bg-white p-10 text-center">
        ...
        <p className="mt-2 text-sm text-muted">Add products to cart before placing your order.</p>
  ...
  return (
    <div className="grid gap-6 sm:gap-8 lg:grid-cols-[2fr_1fr]">
      <section className="rounded-2xl border border-border bg-white p-4 sm:p-6">
        <h1 className="text-2xl font-bold sm:text-3xl">Secure Checkout</h1>
        <p className="mt-2 text-sm text-muted">Complete your order details and place your gift in minutes.</p>

        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-5">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Full Name" error={errors.fullName?.message}>
              <input {...register("fullName")} className="min-h-11 w-full rounded-lg border border-border px-3 py-2 text-sm" />
            </Field>
            <Field label="Email" error={errors.email?.message}>
              <input {...register("email")} className="min-h-11 w-full rounded-lg border border-border px-3 py-2 text-sm" />
            </Field>
            <Field label="Phone" error={errors.phone?.message}>
              <input {...register("phone")} className="min-h-11 w-full rounded-lg border border-border px-3 py-2 text-sm" />
            </Field>
            <Field label="City" error={errors.city?.message}>
              <input {...register("city")} className="min-h-11 w-full rounded-lg border border-border px-3 py-2 text-sm" />
            </Field>
            <Field label="Address" error={errors.address?.message} className="sm:col-span-2">
              <textarea {...register("address")} className="w-full rounded-lg border border-border px-3 py-2 text-sm" rows={3} />
            </Field>
            <Field label="Pin Code" error={errors.pinCode?.message}>
              <input {...register("pinCode")} className="w-full rounded-lg border border-border px-3 py-2 text-sm" />
            </Field>
          ...
          <Field label="Payment Method" error={errors.payment?.message}>
            <select {...register("payment")} className="min-h-11 w-full rounded-lg border border-border px-3 py-2 text-sm">
          ...
          <button
            ...
            className="min-h-11 w-full rounded-lg bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground"
          >
      ...
      <aside className="h-fit rounded-2xl border border-border bg-surface p-4 sm:p-5 lg:sticky lg:top-24">
        ...
        <p className="mt-3 text-xs text-muted">{promo.message}</p>
      </aside>
    </div>
  );
}

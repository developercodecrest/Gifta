...
export default function Home() {
  ...
  return (
    <div className="space-y-16 lg:space-y-20">
      <section className="grid gap-8 rounded-3xl border border-border bg-gradient-to-br from-surface to-background p-7 sm:p-8 lg:grid-cols-2 lg:p-12">
        <div className="space-y-7">
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/store" className="rounded-lg bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground transition hover:opacity-90">
              Explore Store
            </Link>
            <Link href="/store?sort=rating" className="rounded-lg border border-border bg-white px-5 py-3 text-sm font-semibold transition hover:bg-surface">
              Top Rated Gifts
            </Link>
          </div>
        </div>
...
function CategoryTile({ title, href }: { title: string; href: string }) {
  return (
    <Link href={href} className="rounded-2xl border border-border bg-white p-6 transition hover:-translate-y-0.5 hover:shadow-md">

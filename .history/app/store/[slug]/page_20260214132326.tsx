...
export default async function ProductPage({
  ...
  return (
    <div className="space-y-10">
      ...
      <RecentSection currentId={product.id} />
    </div>
  );
}

function RecentSection({ currentId }: { currentId: string }) {
  const recent = products.filter((item) => item.id !== currentId).slice(0, 4);

  return (
    <section>
      <h2 className="mb-4 text-2xl font-bold">Recently trending</h2>
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {recent.map((item) => (
          <ProductCard key={item.id} product={item} />
        ))}

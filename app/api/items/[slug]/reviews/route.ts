...
import { getItemReviews } from "@/lib/server/ecommerce-service";
...
export async function GET(
  ...
  try {
    ...
    const result = await getItemReviews(slug, parsed.data.page, parsed.data.pageSize);
    if (!result) {
    ...
    return serverError("Unable to fetch item reviews", error);
  }
}

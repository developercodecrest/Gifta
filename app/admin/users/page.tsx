...
export default async function AdminUsersPage() {
  const role = await ensureAdminAccess("users");
  ...
  return (
    <div className="space-y-6">
      <header className="rounded-xl border border-border bg-card p-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">View customer and owner profiles. Active role: {roleLabels[role]}.</p>
      </header>

      <div className="grid gap-3 md:grid-cols-2">
        {users.map((user) => (
          <Card key={user.profileKey}>
            <CardContent className="space-y-2 p-4">
              <p className="font-semibold">{user.fullName}</p>
              <p className="text-sm text-muted-foreground">{user.email}</p>
              <p className="text-xs text-muted-foreground">Profile Key: {user.profileKey}</p>

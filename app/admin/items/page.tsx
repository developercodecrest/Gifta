...
export default async function AdminItemsPage() {
  const role = await ensureAdminAccess("items");

  const items = await getAdminItems().catch(() => ({ items: [] as Array<{ id: string; name: string; category: string; slug: string; minOrderQty?: number; maxOrderQty?: number; bestOffer?: { price: number; store?: { name: string } }; offerCount: number }> }));
  ...
  return (
    <div className="space-y-6">
      <header className="rounded-xl border border-border bg-card p-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Review product coverage and vendor offer depth. Active role: {roleLabels[role]}.</p>
      </header>

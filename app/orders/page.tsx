  ...
  return (
    <div className="space-y-6">
      <header className="surface-mesh soft-shadow rounded-4xl border border-white/70 p-6 sm:p-8 lg:p-10">
        ...
        <p className="mt-3 max-w-2xl text-sm text-muted-foreground sm:text-base">Review placed orders, check delivery progress, and revisit previous gifting choices from a cleaner, more premium order surface.</p>
      </header>
      ...
      {orders.length === 0 ? (
        <Card className="rounded-4xl border-dashed border-[#e5c9bb] bg-white/70">
          <CardContent className="p-8 text-center">
            ...
            <p className="mt-4 text-sm text-muted-foreground">No orders yet. Complete checkout to see your order history here.</p>
              ...
              <CardContent className="p-5 sm:p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">{order.itemsSummary || `${order.itemCount} item(s)`}</p>
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-3">
                  <div className="rounded-3xl border border-white/70 bg-white/70 p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Placed on</p>
                ...
                <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
                  <p className="flex items-center gap-2 text-sm text-muted-foreground"><Truck className="h-4 w-4 text-primary" />Delivery flow stays visible from checkout through fulfilment.</p>
                  <Button asChild variant="outline" className="border-[#efc9ba] bg-white/80">

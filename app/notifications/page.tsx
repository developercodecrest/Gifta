...
export default function NotificationsPage() {
  ...
  return (
    <div className="space-y-6">
      <header className="rounded-2xl border border-border bg-card p-5 sm:p-7">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-[#5f5047]">Track your order and payment updates in one place.</p>
          </div>
      ...
      {isLoading ? <p className="text-sm text-[#5f5047]">Loading notifications...</p> : null}
      {error ? <p className="text-sm text-destructive">{error}</p> : null}

      {!isLoading && !error && payload.notifications.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="p-8 text-center">
            <p className="text-sm text-[#5f5047]">No notifications yet.</p>
          </CardContent>
        </Card>
      ) : null}
          ...
          {payload.notifications.map((notification) => (
            <Card key={notification.id} className={notification.isRead ? "opacity-75" : ""}>
              <CardContent className="p-5">
                ...
                <p className="mt-2 text-sm text-[#5f5047]">{notification.message}</p>
                <div className="mt-3 flex items-center justify-between gap-3 text-xs text-[#74655c]">

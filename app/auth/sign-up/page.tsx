...
type OtpRequestPayload = {
  ...
  error?: { message?: string; details?: { retryAfterMs?: number } };
};
...
function SignUpContent() {
  ...
  const [signingUpMobile, setSigningUpMobile] = useState(false);
  ...
  const hasExistingAccount = async (input: { email?: string; phone?: string }) => {
    ...
    const payload = (await response.json().catch(() => ({}))) as AccountExistsPayload;
    return Boolean(response.ok && payload.success && payload.data?.exists);
  };
  ...
  const saveProfileAndContinue = async () => {
    ...
    const fullNameValue = profileName.trim();
    const phoneValue = normalizePhone(profilePhone);
  ...
  const onRequestOtp = async () => {
    ...
    try {
      ...
      const accountExists = await hasExistingAccount({ email: email.trim() });
      if (accountExists) {
        setError("Account already exists for this email. Please sign in.");
        return;
      }

      const { response, result: payload } = await requestOtp({ email: email.trim() });
  ...
  const onRequestMobileOtp = async () => {
    ...
    if (!email.trim()) {
      setMobileError("Email is required to create a new account.");
      return;
    }
    ...
    try {
      ...
      const accountExists = await hasExistingAccount({ email: email.trim() });
      if (accountExists) {
        setMobileError("Account already exists for this email. Please sign in.");
        return;
      }
      ...
      setMobileStatus(payload.data?.message ?? "OTP sent. Check your email inbox.");
  ...
  const onOtpSignup = async () => {
    ...
    try {
      ...
      const authResult = await finishTokenSignIn(authUser, { redirectNow: false });
      if (!authResult.ok) {
      ...
      const resolvedName = authUser.fullname?.trim() || fullName.trim() || email.split("@")[0] || "";
      const resolvedPhone = authUser.phone ?? normalizePhone(phone);
  ...
  const onMobileOtpSignup = async () => {
    setMobileError(null);
    setMobileStatus(null);

    const normalizedPhone = normalizePhone(phone);
    if (!email.trim() || !mobileOtp.trim() || !normalizedPhone) {
      setMobileError("Email, mobile number and OTP are required.");
      return;
    }

    try {
      setSigningUpMobile(true);
      const response = await fetch("/api/auth/mobile/sign-in", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      setPostAuthPath(authResult.nextPath ?? callbackUrl);
      const resolvedName = authUser.fullname?.trim() || fullName.trim() || email.split("@")[0] || "";
      const resolvedPhone = authUser.phone ?? normalizedPhone;
      setProfileName(resolvedName);
      setProfilePhone(resolvedPhone);
      setProfileReceiverName(resolvedName);
      setProfileReceiverPhones([resolvedPhone || ""]);
  ...
  return (
    <div className="space-y-6 p-2">
      <div>
        ...
        <p className="mt-1 text-sm text-[#5f5047]">Sign up with OTP or Google in one tap.</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        {activeTab === "email" ? (
          ...
          <Field label="Full Name (Optional)">
            <Input type="text" placeholder="Your full name" value={fullName} onChange={(event) => setFullName(event.target.value)} />
          </Field>
        ...
        {activeTab === "mobile" ? (
          ...
          <p className="text-xs text-[#5f5047]">OTP will be sent to your email and this mobile will be saved to your profile.</p>
          ...
          {mobileOtpSent ? (
            <>
              ...
              <Button type="button" className="w-full" onClick={onMobileOtpSignup} disabled={signingUpMobile}>
                {signingUpMobile ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />}
                {signingUpMobile ? "Creating account..." : "Verify OTP & Sign up"}
              </Button>
            </>
          ) : null}
      ...
      <div className="text-sm text-[#5f5047]">
